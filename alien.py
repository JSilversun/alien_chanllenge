...
class CommandCenter:
    def __init__(self, name, row, col,z):
        self.name=name
        self.scale=scale
...
def destroy_layer(m,command_centers):
    ...
    for i,command_center in enumerate(layer_centers):
        if command_center.missing_cells==0:
            result+=str(command_center.name)+":"+str(format(command_center.center_y,'.3f'))+","+str(format(command_center.center_x,'.3f'))
...
for i in range(1,n+1):
    ...
    while len(command_centers)>0:
        destroy_layer(m,command_centers)
